...
@views function vizme2D_mpi(nprocs)
    ...
    for it=1:100
        ...
        for ipx = 1:nprocs[1]
            for ipy = 1:nprocs[2]
                file = matopen("mpi2D_gpu_out_C_$(ip-1)_it_$(it).mat"); C_loc = read(file, "C"); close(file)
        ...
        frame(anim, heatmap(C'; c=:davos, title="diffusion 2D MPI (it=$(it))", opts...))
    end
    gif(anim, "diffusion_2D_mpi_gpu.gif", fps=5)
    return
end
